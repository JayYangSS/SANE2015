...
void CRTree::makeLeaf(const std::vector<std::vector<const PatchFeature*> >& TrainSet, float pnratio, int node) {
	...
	//for check
	FILE* fp;
	fp = fopen("pnratio_samplePatch20.txt", "a");
	if (fp == NULL){
	...
	fprintf(fp, "pfg: %f		pnratio: %f	positive leaf offset: %d	negative leaf offset: %d	vCenter size: %d\n", ptL->pfg, pnratio, TrainSet[1].size(), TrainSet[0].size(), ptL->vCenter.size());
...
void CRTree::evaluateTest(std::vector<std::vector<IntIndex> >& valSet, const int* test, const std::vector<std::vector<const PatchFeature*> >& TrainSet) {
	for(unsigned int l=0;l<TrainSet.size();++l) {
		...
		for(unsigned int i=0;i<TrainSet[l].size();++i) {
			...
			// pointer to channel
			CvMat* ptC = TrainSet[l][i]->vPatch[test[4]];	//test[4] : feature notation
			// get pixel values 
...
void LeafNode::show(int delay, int width, int height) {
	...
	if(vCenter.size()>0) {
		...
		for(unsigned int c = 0; c<iShow.size(); ++c) {
			...
			////smooth
			//cvSmooth(iShow[c], iShow[c], CV_GAUSSIAN, 3);
			//cvConvertScale(iShow[c], iShow[c], 255);
